...
def transcribe_and_diarize(audio_path: str, language: str, hf_token: str = None) -> str:
    """
    ...
    try:
        # Prioritize token from argument, then environment variable, then cached login.
        ...
        if auth_token:
            ...
            print("No Hugging Face token provided. Relying on cached login from 'huggingface-cli login'.")
        ...
        print("Please ensure you have accepted the user agreements on Hugging Face Hub for the models.")
        print("If not providing a token, ensure you have logged in via 'huggingface-cli login'.")
        print("------------------------\n")
        sys.exit(1)
...
def main():
    ...
    try:
        transcript = ""
        if args.diarize:
            transcript = transcribe_and_diarize(audio_path, args.language, args.hf_token)
        else:
